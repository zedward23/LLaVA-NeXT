...
PREV_STAGE_CHECKPOINT="checkpoints/onevision/llava-onevision-qwen2-7b-si" # replace it with your last checkpoint training from mid stage
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --bf16 True \
    --run_name $RUN_NAME \
    --output_dir checkpoints/onevision/dtc_lora_7b_$RUN_NAME \
    --num_train_epochs 10  \
